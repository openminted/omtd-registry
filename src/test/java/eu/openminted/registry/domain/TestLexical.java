...
public class TestLexical {
	...
	public static Collection<Object[]> data() {
		File folder = new File("src/test/resources/lexical/active");
	...
	public TestLexical(File testXML) {
	...
	public static void before() throws JAXBException {
        ...
        for (int i = 0; i < numThreads; ++i) {
        	...
        	validator.get(i).setSchemaSource(Input.fromFile("src/main/xsd/OMTD-SHARE-LexicalConceptualResource.xsd").build());
        }
	}
	...
	public void test() throws JAXBException, FileNotFoundException {
		...
		marshaller.get(id).marshal(lexicalConceptual, sw);
		
		Source original = Input.fromFile(testXML).build();
		Source marshaled = Input.fromJaxb(lexicalConceptual).build();
		...
		if(diff.hasDifferences()) {
			...
			marshaller.get(id).marshal(lexicalConceptual, System.out);
