...
public class TestCorpus {
	...
	public static Collection<Object[]> data() {
		File folder = new File("src/test/resources/corpus/active");
	...
	public TestCorpus(File testXML) {
	...
	public static void before() throws JAXBException {
        ...
        for (int i = 0; i < numThreads; ++i) {
        	...
        	validator.get(i).setSchemaSource(Input.fromFile("src/main/xsd/OMTD-SHARE-Corpus.xsd").build());
        }
	}
	...
	public void test() throws JAXBException, FileNotFoundException {
		...
		Corpus corpus = (Corpus) unmarshaller.get(id).unmarshal(in);

		
		StringWriter sw = new StringWriter();
		marshaller.get(id).marshal(corpus, sw);
		
		Source original = Input.fromFile(testXML).build();
		Source marshaled = Input.fromJaxb(corpus).build();
		...
		if(diff.hasDifferences()) {
			...
			marshaller.get(id).marshal(corpus, System.out);
