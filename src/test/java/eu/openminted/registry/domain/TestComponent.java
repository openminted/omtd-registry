...
public class TestComponent {
	...
	public static Collection<Object[]> data() {
		File folder = new File("src/test/resources/component/active");
	...
	public TestComponent(File testXML) {
	...
	public static void before() throws JAXBException {
        ...
        for (int i = 0; i < numThreads; ++i) {
        	...
        	validator.get(i).setSchemaSource(Input.fromFile("src/main/xsd/OMTD-SHARE-Component.xsd").build());
        }
	}
	...
	public void test() throws JAXBException, FileNotFoundException {
		...
		Component component = (Component) unmarshaller.get(id).unmarshal(in);

		
		StringWriter sw = new StringWriter();
		marshaller.get(id).marshal(component, sw);
		
		Source original = Input.fromFile(testXML).build();
		Source marshaled = Input.fromJaxb(component).build();
		...
		if(diff.hasDifferences()) {
			...
			marshaller.get(id).marshal(component, System.out);
