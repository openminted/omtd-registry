...
import eu.openminted.registry.domain.operation.*;
import eu.openminted.registry.service.aai.UserInfoAAIRetrieve;
import eu.openminted.registry.service.generate.*;
import eu.openminted.registry.service.mail.JavaMailer;
...
public class OperationHandler {
    ...
    @JmsListener(containerFactory = "jmsQueueListenerContainerFactory", destination = "${jms.workflows" +
        ...
        synchronized (OperationServiceImpl.class) {
            try {
                ...
                if (workflowExeMsg.getWorkflowStatus().equalsIgnoreCase(ExecutionStatus.Status.PENDING.toString())) {
                    logger.info("Ignoring and waiting PENDING for operation with id " + workflowExeMsg
                ...
                // Set a workflow experiment to resumed, failed, paused, ie update an operation document
                    ...
                    if (workflowExeMsg.getWorkflowExecutionID() == null) {
                        throw new NullPointerException("Missing elements in WorkflowExecutionStatusMessage for status" +
                                " " + workflowExeMsg.getWorkflowStatus().toUpperCase());
                    }
                    ...
                    logger.info("Update Operation " + operation.getId() + " to status " + workflowExeMsg
    ...
    private void casePending(WorkflowExecutionStatusMessage workflowExeMsg) {
        if (workflowExeMsg.getWorkflowExecutionID() == null) {
            throw new NullPointerException("Missing elements in WorkflowExecutionStatusMessage for status " +
                    ExecutionStatus.Status.PENDING.toString());
        }
        ...
        Future<String> operationString = parserPool.serialize(operation, ParserServiceTypes.JSON);
        logger.info("Add Operation " + operation.getId() + " to status " + workflowExeMsg.getWorkflowStatus()
                .toUpperCase());
    ...
    private void caseRunning(WorkflowExecutionStatusMessage workflowExeMsg) throws ResourceNotFoundException {
        if (workflowExeMsg.getWorkflowExecutionID() == null) {
            throw new NullPointerException("Missing elements in WorkflowExecutionStatusMessage for status " +
                    ExecutionStatus.Status.RUNNING.toString());
        }
        // Get operation object from registry
        Operation operation = operationService.get(workflowExeMsg.getWorkflowExecutionID());
        ...
        // Update operation to registry
        Future<String> operationString = parserPool.serialize(operation, ParserServiceTypes.JSON);
        logger.info("Update Operation " + operation.getId() + " to status " + workflowExeMsg.getWorkflowStatus()
                .toUpperCase());
        operationService.update(operation);
    }

    private void caseFinished(
            WorkflowExecutionStatusMessage workflowExeMsg) throws IOException, ResourceNotFoundException,
            NullPointerException {

        if (workflowExeMsg.getWorkflowExecutionID() == null || workflowExeMsg.getResultingCorpusID() == null) {
            logger.debug("Missing elements in WorkflowExecutionStatusMessage for status " + ExecutionStatus.Status
                    .FINISHED.toString());
            throw new NullPointerException("Missing elements in WorkflowExecutionStatusMessage for status " +
                    ExecutionStatus.Status.FINISHED.toString());
        }
        ...
        eu.openminted.registry.domain.Component workflowMeta = (eu.openminted.registry.domain.Component)
                applicationService.get(operation.getComponent());
        ...
        if (workflowMeta.getComponentInfo().getOutputResourceInfo() != null) {
            if (workflowMeta.getComponentInfo().getOutputResourceInfo().getProcessingResourceType().equals
                    (ProcessingResourceTypeEnum.LANGUAGE_DESCRIPTION)) {
                // Generate output language description metadata
                logger.info("Generating metadata for language description from experiment " + workflowExeMsg
                        .getWorkflowExecutionID());
                eu.openminted.registry.domain.LanguageDescription outputLanguageMeta = languageMetadataGenerator
                        .generateLanguageDescriptionMetadata(operationCorpus.getInput(),
            ...
            } else if (workflowMeta.getComponentInfo().getOutputResourceInfo().getProcessingResourceType().equals
                    (ProcessingResourceTypeEnum.LEXICAL_CONCEPTUAL_RESOURCE)) {
                // Generate output lcr metadata
                logger.info("Generating metadata for lexical/conceptual resource from experiment " + workflowExeMsg
                        .getWorkflowExecutionID());
                eu.openminted.registry.domain.Lexical outputLexicalMeta = lexicalMetadataGenerator
                        .generateLanguageConceptualResourceMetadata(operationCorpus.getInput(),
                        operation.getComponent(), operation.getPerson(), workflowExeMsg.getResultingCorpusID());
                ...
                logger.info("Generating metadata for annotated corpus from experiment " + workflowExeMsg
                        .getWorkflowExecutionID());
                eu.openminted.registry.domain.Corpus outputCorpusMeta = corpusMetadataGenerator
                        .generateAnnotatedCorpusMetadata(operationCorpus.getInput(),
                        operation.getComponent(), operation.getPerson(), workflowExeMsg.getResultingCorpusID());
        ...
        try {
            ...
            javaMailer.sendEmail(
                    ...
                    "Dear " + givenName + " " + surname + ",\n" +
                            "\n" +
                            "  The application " + workflowMeta.getComponentInfo().getIdentificationInfo()
                            .getResourceNames().get(0).getValue() + " you executed in OpenMinTeD has been " +
                            "successfully completed with output\" " + outputOmtdId + " \".\n" +
    ...
    private void caseFailed(WorkflowExecutionStatusMessage workflowExeMsg) throws ResourceNotFoundException {
        if (workflowExeMsg.getWorkflowExecutionID() == null || workflowExeMsg.getError() == null) {
            throw new NullPointerException("Missing elements in WorkflowExecutionStatusMessage for status " +
                    ExecutionStatus.Status.FAILED.toString());
        }
        ...
        try {
            ...
            javaMailer.sendEmail(
                    ...
                    "Dear " + givenName + " " + surname + ",\n" +
                            "\n" +
                            " The application " + workflowMeta.getComponentInfo().getIdentificationInfo()
                            .getResourceNames().get(0).getValue() + " you executed in OpenMinTeD has failed." +
                            "\n For more details, visit the <a href=\"" + registryHost + "\">OpenMinTeD</a> site.\n" +
                            "\n" +
                            "Best regards,\n" +
                            "The OpenMinTeD team");
