        ...
        for (Facet facet : facets) {
            for (Value value : facet.getValues()) {
                switch (facet.getField()) {
                    case "language":
                        value.setLabel(getLanguageLabel(value.getValue()));
                        break;
                    case "licence":
                        value.setLabel(mappings.get("licence").getProperty(sanitize(value.getValue())));
                        break;
                    case "function":
                    case "annotationType":
                    case "dataFormat":
                        value.setLabel(mappings.get("ontology").getProperty(sanitize(value.getValue())));
                        break;
                    default:
                        value.setLabel(capitalize(value.getValue()));
                }
            }
        }
    }
